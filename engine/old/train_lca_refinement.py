...
def main(config_override=None):
    config = {
        "experiment_name": "convnextv2_tiny_512x512_Refinement",
        "base_model_ckpt": "/workspace/Collateral-Coronary-Vessels-XAI/checkpoints/ConvNextV2_Tiny_DCUnet_512x512/best_model.pth", 
        "data": {
        ...
        "model": {
            "base_arch": "dcn",
            "base_encoder": "tu-convnextv2_tiny",
            "refinement_encoder": "tu-convnextv2_tiny", # Folosim un encoder puternic (același ca la baza)
        },
        "training": {
            "epochs": 100,
            "lr": 1e-4,
    ...
    base_model = MultiTaskTargetedUNet(
        arch=config["model"]["base_arch"],
        encoder_name=config["model"]["base_encoder"],
    ...
    if os.path.exists(last_path):
        print(f"\n[INFO] Reluare antrenament Refiner din checkpoint-ul existent: {last_path}")
    ...
    for epoch in range(start_epoch, config["training"]["epochs"]):
        ...
        print(f"Epoch {epoch + 1}/{config['training']['epochs']} | "
              ...
              f"{'(New Best)' if metrics['ref_global'] == best_val_f1 else ''}{patience_msg}")
              
        if patience > 0 and epochs_no_improve >= patience:
            print(f"\n[EARLY STOPPING] Nicio imbunatatire timp de {patience} epoci. Se opreste rafinarea.")
            break
