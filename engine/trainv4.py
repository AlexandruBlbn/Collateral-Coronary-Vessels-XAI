...
class VesselNetV3EfficientNet(nn.Module):
	def __init__(
		self,
		in_chans: int = 4,
		num_classes: int = 1,
		...
		try:
			...
			# Some CNN-style timm backbones do not accept img_size; retry without it.
		...
		self.embedding_head = nn.Sequential(
			nn.Conv2d(self.enc_channels[3], embedding_dim, kernel_size=1, bias=False),
	...
	def _ensure_nchw(self, feat: torch.Tensor, expected_channels: int, stage_name: str) -> torch.Tensor:
		...
		# CNN backbones typically return NCHW; some transformer backbones (e.g., Swin) return NHWC.
	...
	def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
		...
		deep3 = F.interpolate(self.deep_head3(d3), size=(h, w), mode="bilinear", align_corners=False)
		deep2 = F.interpolate(self.deep_head2(d2), size=(h, w), mode="bilinear", align_corners=False)
		encoder_embeddings = F.normalize(self.embedding_head(f4), dim=1)
...
def focal_tversky_loss(pred, target, alpha: float = 0.3, beta: float = 0.7, gamma: float = 2.0, eps: float = 1e-6):
	pred = pred.float()
	target = target.float()
	tp = torch.sum(pred * target, dim=(2, 3))
	fp = torch.sum(pred * (1.0 - target), dim=(2, 3))
	fn = torch.sum((1.0 - pred) * target, dim=(2, 3))
	tversky = (tp + eps) / (tp + alpha * fp + beta * fn + eps)
	return torch.pow((1.0 - tversky), gamma).mean()


def _sample_embeddings_and_labels(
	encoder_embeddings: torch.Tensor,
	target_mask: torch.Tensor,
	max_samples: int,
) -> Tuple[torch.Tensor, torch.Tensor]:
	b, c, h, w = encoder_embeddings.shape
	mask_small = F.interpolate(target_mask.float(), size=(h, w), mode="nearest")

	z = encoder_embeddings.permute(0, 2, 3, 1).reshape(-1, c).float()
	y = (mask_small.reshape(-1) > 0.5).long()

	if z.shape[0] <= max_samples:
		return F.normalize(z, dim=1), y

	fg_idx = torch.nonzero(y == 1, as_tuple=False).squeeze(1)
	bg_idx = torch.nonzero(y == 0, as_tuple=False).squeeze(1)

	half = max(1, int(max_samples) // 2)
	n_fg = min(fg_idx.numel(), half)
	n_bg = min(bg_idx.numel(), max_samples - n_fg)

	remaining = max_samples - (n_fg + n_bg)
	if remaining > 0:
		fg_left = max(0, fg_idx.numel() - n_fg)
		bg_left = max(0, bg_idx.numel() - n_bg)
		if fg_left >= bg_left:
			n_fg += min(remaining, fg_left)
		else:
			n_bg += min(remaining, bg_left)

	sel_parts: List[torch.Tensor] = []
	if n_fg > 0:
		perm_fg = torch.randperm(fg_idx.numel(), device=fg_idx.device)[:n_fg]
		sel_parts.append(fg_idx[perm_fg])
	if n_bg > 0:
		perm_bg = torch.randperm(bg_idx.numel(), device=bg_idx.device)[:n_bg]
		sel_parts.append(bg_idx[perm_bg])

	if not sel_parts:
		sel = torch.arange(max_samples, device=z.device)
	else:
		sel = torch.cat(sel_parts, dim=0)
		sel = sel[torch.randperm(sel.numel(), device=sel.device)]

	z_sel = z.index_select(0, sel)
	y_sel = y.index_select(0, sel)
	return F.normalize(z_sel, dim=1), y_sel


def supervised_contrastive_loss(z: torch.Tensor, y: torch.Tensor, temperature: float = 0.1, eps: float = 1e-8) -> torch.Tensor:
	if z.numel() == 0 or y.numel() < 2:
		return z.new_tensor(0.0)

	z = F.normalize(z, dim=1)
	sim = torch.matmul(z, z.t()) / max(1e-6, float(temperature))
...
def _build_foreground_prototypes(fg_embeddings: torch.Tensor, num_prototypes: int) -> torch.Tensor:
	if fg_embeddings.numel() == 0:
		return fg_embeddings

	fg_embeddings = F.normalize(fg_embeddings, dim=1)
	k = max(1, min(int(num_prototypes), fg_embeddings.shape[0]))
	if k == 1:
		return fg_embeddings.mean(dim=0, keepdim=True)

	centers: List[torch.Tensor] = []
	start_idx = torch.randint(0, fg_embeddings.shape[0], (1,), device=fg_embeddings.device).item()
	first_center = fg_embeddings[start_idx : start_idx + 1]
	centers.append(first_center)

	min_dist = 1.0 - torch.matmul(fg_embeddings, first_center.t()).squeeze(1)
	for _ in range(1, k):
		next_idx = torch.argmax(min_dist).item()
		center = fg_embeddings[next_idx : next_idx + 1]
		centers.append(center)
		cur_dist = 1.0 - torch.matmul(fg_embeddings, center.t()).squeeze(1)
		min_dist = torch.minimum(min_dist, cur_dist)
...
def prototypical_contrastive_loss(
	...
	prototypes = _build_foreground_prototypes(fg, num_prototypes=num_prototypes)

	fg_dist = 1.0 - torch.matmul(fg, prototypes.t())
	pos_term = fg_dist.min(dim=1).values.pow(2).mean()

	bg_dist = 1.0 - torch.matmul(bg, prototypes.t())
	bg_min_dist = bg_dist.min(dim=1).values
	neg_hinge = F.relu(float(margin) - bg_min_dist).pow(2)

	if neg_hinge.numel() == 0:
		neg_term = z.new_tensor(0.0)
	elif hard_negative_ratio < 1.0:
		k = max(1, int(float(hard_negative_ratio) * neg_hinge.numel()))
		neg_term = torch.topk(neg_hinge, k=k, largest=True).values.mean()
	else:
		neg_term = neg_hinge.mean()

	return float(positive_weight) * pos_term + float(negative_weight) * neg_term


def supervised_prototypical_contrastive_loss(
	encoder_embeddings: torch.Tensor,
	target_mask: torch.Tensor,
	temperature: float,
	num_prototypes: int,
	...
	z, y = _sample_embeddings_and_labels(
		encoder_embeddings=encoder_embeddings,
		target_mask=target_mask,
		max_samples=max(64, int(max_samples)),
	)

	sce = supervised_contrastive_loss(z, y, temperature=temperature)
	pcl = prototypical_contrastive_loss(
		z,
		y,
	...
	spcl = float(sce_weight) * sce + float(pcl_weight) * pcl
	return spcl, sce, pcl
...
def ohem_topk_pixels(loss_map: torch.Tensor, target_mask: torch.Tensor, keep_ratio: float = 0.25, min_kept: int = 1024):
	b = loss_map.shape[0]
	...
	for i in range(b):
		...
		if neg_loss.numel() > 0:
			...
			if pos_loss.numel() > 0:
				selected = torch.cat([pos_loss, hard_neg], dim=0)
			else:
				selected = hard_neg
		else:
			selected = pos_loss if pos_loss.numel() > 0 else cur_loss
...
def annealed_bce_boundary_weights(
	...
	if schedule_epochs is None or int(schedule_epochs) <= 0:
		anneal_epochs = int(total_epochs)
	else:
		anneal_epochs = min(int(schedule_epochs), int(total_epochs))
	anneal_epochs = max(1, anneal_epochs)

	t_raw = min(float(epoch) / float(max(1, anneal_epochs - 1)), 1.0)
	t = t_raw ** max(1e-6, float(power))
...
class VesselHybridLoss(nn.Module):
	def __init__(self, config: Dict):
		...
		self.sdm_weight = float(loss_cfg.get("sdm_weight", 0.25))
		self.focal_tversky_weight = float(loss_cfg.get("focal_tversky_weight", 0.2))
		self.cldice_iters = int(loss_cfg.get("cldice_iters", 5))
		self.tversky_alpha = float(loss_cfg.get("tversky_alpha", 0.3))
		self.tversky_beta = float(loss_cfg.get("tversky_beta", 0.7))
		self.tversky_gamma = float(loss_cfg.get("tversky_gamma", 2.0))
		self.spcl_weight = float(loss_cfg.get("spcl_weight", 0.2))
		self.sce_weight = float(loss_cfg.get("sce_weight", 1.0))
		self.pcl_weight = float(loss_cfg.get("pcl_weight", 1.0))
		self.spcl_temperature = float(loss_cfg.get("spcl_temperature", 0.1))
		self.spcl_num_prototypes = int(loss_cfg.get("spcl_num_prototypes", 2))
		self.spcl_margin = float(loss_cfg.get("spcl_margin", 0.7))
		self.spcl_pos_weight = float(loss_cfg.get("spcl_pos_weight", 1.0))
		self.spcl_neg_weight = float(loss_cfg.get("spcl_neg_weight", 1.0))
		self.spcl_hard_neg_ratio = float(loss_cfg.get("spcl_hard_neg_ratio", 0.25))
		self.spcl_max_samples = int(loss_cfg.get("spcl_max_samples", 1024))
		...
		self.anneal_epochs = int(loss_cfg.get("anneal_epochs", max(1, int(config["training"].get("epochs", 1)) // 3)))
		self.anneal_power = float(loss_cfg.get("anneal_power", 0.7))
	...
	def forward(self, outputs, target_mask, target_sdm, epoch: int, total_epochs: int):
		...
		encoder_embeddings = outputs.get("encoder_embeddings", None)
		...
		if self.spcl_weight > 0.0 and encoder_embeddings is not None:
			spcl_loss, sce_loss, pcl_loss = supervised_prototypical_contrastive_loss(
				encoder_embeddings=encoder_embeddings,
				target_mask=target_mask,
				temperature=self.spcl_temperature,
				num_prototypes=self.spcl_num_prototypes,
				margin=self.spcl_margin,
				positive_weight=self.spcl_pos_weight,
				negative_weight=self.spcl_neg_weight,
				hard_negative_ratio=self.spcl_hard_neg_ratio,
				max_samples=self.spcl_max_samples,
				sce_weight=self.sce_weight,
				pcl_weight=self.pcl_weight,
			)
		else:
			spcl_loss = seg_logits.new_tensor(0.0)
			sce_loss = seg_logits.new_tensor(0.0)
			pcl_loss = seg_logits.new_tensor(0.0)
		...
		bce_w, boundary_w = annealed_bce_boundary_weights(
			...
			schedule_epochs=self.anneal_epochs,
			power=self.anneal_power,
		)
		...
		total = seg_loss + self.sdm_weight * sdm_loss + self.spcl_weight * spcl_loss
		...
		parts = {
			...
			"spcl": float(spcl_loss.detach().item()),
			"sce": float(sce_loss.detach().item()),
			"pcl": float(pcl_loss.detach().item()),
...
def train_epoch(model, dataloader, criterion, optimiser, epoch, config, scaler=None):
	...
	for batch_idx, (images, masks, sdm, _) in pbar:
		...
		for k in ["seg", "bce_ohem", "boundary_ohem", "cldice_loss", "focal_tversky", "spcl", "sce", "pcl", "deep_sup", "sdm", "w_bce", "w_boundary"]:
		...
		pbar.set_postfix(
			{
				...
				"ftv": running["focal_tversky"] / denom,
				"spcl": running["spcl"] / denom,
			}
		)
...
def validate_epoch(model, dataloader, criterion, epoch, config, threshold=0.5, tb_writer=None, split_name="Val"):
	...
	with torch.no_grad():
		...
		for batch_idx, (images, masks, sdm, _) in pbar:
			...
			batch_cldice = 1.0 - float(soft_cldice_loss(preds, masks, iter_=int(config["loss"].get("cldice_iters", 5))).item())
			...
			if tb_writer is not None and batch_idx == 0:
...
def trainScript(
	...
	for epoch in range(start_epoch, num_epochs):
		...
		tb_writer.add_scalar("Loss/train_focal_tversky", train_stats["focal_tversky"], epoch)
		tb_writer.add_scalar("Loss/train_spcl", train_stats["spcl"], epoch)
		tb_writer.add_scalar("Loss/train_sce", train_stats["sce"], epoch)
		tb_writer.add_scalar("Loss/train_pcl", train_stats["pcl"], epoch)
		...
		tb_writer.add_scalar("Loss/val_focal_tversky", val_stats["focal_tversky"], epoch)
		tb_writer.add_scalar("Loss/val_spcl", val_stats["spcl"], epoch)
		tb_writer.add_scalar("Loss/val_sce", val_stats["sce"], epoch)
		tb_writer.add_scalar("Loss/val_pcl", val_stats["pcl"], epoch)
