...
def train_epoch(model, dataloader, criterion, optimiser, epoch, config, scaler=None):
	...
	for batch_idx, (images, masks, sdm, _) in pbar:
		...
		for k in ["seg", "bce_ohem", "boundary_ohem", "cldice_loss", "focal_tversky", "deep_sup", "sdm", "w_bce", "w_boundary"]:
...
if __name__ == "__main__":
	config = {
		"experiment_name": "trainv3_convnextv2_small",
		"logging": {
		...
		"loss": {
			"recipe": "OHEM BCE + Faster Boundary Annealing + clDice + FocalTversky + Deep Supervision + SDM",
			"ohem_ratio": 0.25,
			"ohem_min_kept": 2048,
			"cldice_weight": 0.2,
